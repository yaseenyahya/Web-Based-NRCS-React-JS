...
import {
  ...
  setEditItemModalLoading,
  setEditItemModalFolderData,
  setEditItemModalSubject,
  setEditItemModalContent,
  resetEditItemModal,
  setEditItemModalMainTopContainerHeight,
  setEditItemModalMainMiddleContainerHeight,
  setEditItemModalMainBottomContainerHeight,
  editItemToFolder,
...
import { setUploadAttachmentsModalToggle,resetUploadAttachmentsModal } from "../../store/actions/UploadAttachmentsModalActions";
...
const EditItemModal = (props) => {
  ...
  const handleSubmit = () => {
    if (props.editItemModalLoading) return;
    let isValid = true;
    if (!subjectValidate()) {
      isValid = false;
    }
    ...
    if (isValid) props.editItemToFolder(enqueueSnackbar);
  };
  useEffect(() => {
  ...
  return (
    <Dialog
      ...
      open={props.editItemModalToggle}
      onEntered={() => {
        setMainTopMiddleBottomContainerHeight();
      }}
      onClose={() => {
        props.setEditItemModalToggle(false);
        props.resetEditItemModal();
      ...
      <div className={classes.hiddenContainer}>
        <div ref={printContainer} className={classes.printContainer}>
          <h1 className={classes.printSubjectContainer}>
            {props.editItemModalSubject}
          </h1>
          <h4 className={classes.printContentContainer}>
            {props.editItemModalContent}
          </h4>
        </div>
      </div>
              ...
              >
                <Box flexGrow={1}>
                  <ValidationTextField
                    ...
                    onInput={(e) => {
                      delaySetMainTopMiddleBottomContainerHeight();
                      props.setEditItemModalSubject(e.target.value);
                    ...
                    notEmpty={true}
                    value={props.editItemModalSubject}
                  />
                </Box>
                <Button
                  onClick={() => {
        ...
        <Container
          ...
          style={{
            height:
              props.userPanelWindowSize.height -
              props.editItemModalMainTopContainerHeight -
              props.editItemModalMainMiddleContainerHeight -
              props.editItemModalMainBottomContainerHeight,
          }}
        >
           <Typography className={classes.durationTextContainer}>Duration: {new includes().calculateTL(   props.editItemModalContent)}</Typography>
         
          <ValidationTextArea
            labelClassName={classes.textAreaLabel}
            className={classes.textArea}
            onChange={(e) => {
              props.setEditItemModalContent(e.target.value);
            }}
            notEmpty={true}
            ...
            value={props.editItemModalContent}
          />
             <CopyButton
            onClick={() => {
        ...
        <Container maxWidth={true} disableGutters={true} ref={mainBottomContainerRef}>
          <Button
            variant="contained"
            type="submit"
            disabled={props.editItemModalLoading}
            className={classes.submitButton}
            onClick={handleSubmit}
          >
            {props.editItemModalLoading && <CircularProgress size={25} />}
            {!props.editItemModalLoading && "Save"}
          </Button>
        </Container>
      </DialogContent>
    </Dialog>
