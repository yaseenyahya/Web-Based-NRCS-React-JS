...
const RundownMenuButton = (props) => {
  ...
  const fontItemSelected = (fontSize) => {
    props.setRundownFontSize(fontSize);
    localStorage.setItem("RundownFontSize",fontSize);
    handleClose();
  }
  const fontSizes = [18, 21, 24, 26];
        ...
        ><span className={classes.itemTypography}>Font Size <ArrowForwardIosIcon className={classes.menuArrow} /></span>
          ...
          >
            ...
            {fontSizes.map((fontSize) => {
              ...
              return <MenuItem
                className={clsx(
                  { [classes.selectedFontSize]: fontSize == props.rundownFontSize },
                  classes.subMenuItem)}
                onClick={() => {
